...
class Agent {
	...
	x; 				///< Abscissa of the agent on the grid. From left (0) to right
	y; 				///< Ordinate of the agent on the grid. From top (0) to bottom
	dir;			///< The orientation of the agent on the grid. 0: east, 1: north, 2:west, 3:south 
	ammo;			///< The remaining ammunitions 
	life;			///< The remaining life. When 0 is reached, no order can be  send until respawn delay is elapsed
	d;				///< The front distance to an obstacle. 0 means nothing visible
	verbose;		///< The verbosity of the logs. 0: None, 1: Error, 2: Warning, 3: Info, 4: Debug
	isConnected; 	///< True if connected to the game server. False if disconnected
    ...
    */
	constructor(id=null, username=null, password=null, arena=null, port=443, server="mqtt.jusdeliens.com", verbose=3, readonly=true) {
		if ( id === null )
			id = window.prompt("Entrer le nom de votre agent (9 caractères max)", "");
		if ( id === null || id === "" )
    ...
    */
	move(dx,dy) {
		...
		{
			if ( dx > 0 )
				this.#agentRequest['dir'] = 0;
			else if ( dx < 0 )
				this.#agentRequest['dir'] = 2;
		}
		if ( dx === 0 )
		{
			if ( dy > 0 )
				this.#agentRequest['dir'] = 3;
			else if ( dy < 0 )
				this.#agentRequest['dir'] = 1;
		}
    ...
    */
    ...
    /** 
     * Set the function to call when a change occurs in the game concerning the agent
     * @param {(Agent) => void}	callback Passes the agent object each time the change occurs
	 * @returns {void} 
    */
	executeOnChange(callback) {
		this.#onChangeCallback = callback;
	}

	/** 
     * Set the function to call periodically each time the agent is updated
     * @param {(Agent) => void}	callback Passes the agent object each time the update occurs
	 * @returns {void} 
    */
	executeOnUpdate(callback) {
